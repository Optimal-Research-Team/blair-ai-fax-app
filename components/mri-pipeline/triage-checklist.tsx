...
function StatusTag({ status }: { status: TagStatus }) {
  const styles: Record<TagStatus, string> = {
    ai_extracted: "bg-emerald-50 text-emerald-700 border-emerald-200/80",
    ai_verified: "bg-sky-50 text-sky-700 border-sky-200/80",
    ai_match: "bg-indigo-50 text-indigo-700 border-indigo-200/80",
    user_labelled: "bg-violet-50 text-violet-700 border-violet-200/80",
    failed: "bg-red-50 text-red-700 border-red-200/80",
    missing: "bg-amber-50/80 text-amber-700 border-amber-200/80",
    new_entry: "bg-sky-50 text-sky-700 border-sky-200/80",
  };
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold border", styles[status])}>
...
function EditableValue({ value, onChange, placeholder, disabled }: { value: string; onChange: (v: string) => void; placeholder?: string; disabled?: boolean }) {
  ...
  if (disabled) return <span className="text-[12px] font-semibold text-foreground">{value}</span>;

  if (!editing) {
    return (
      <button type="button" onClick={(e) => { e.stopPropagation(); setDraft(value); setEditing(true); }}
        className={cn("text-[12px] font-semibold text-left rounded-md px-1.5 py-0.5 -ml-1.5 hover:bg-muted transition-all inline-flex items-center gap-1.5 group max-w-full",
        ...
        <Pencil className="h-3 w-3 text-muted-foreground/40 group-hover:text-muted-foreground transition-opacity shrink-0" />
      </button>
    );
  }
  ...
  return (
    <Input ref={inputRef} value={draft} onChange={(e) => setDraft(e.target.value)}
      ...
      onClick={(e) => e.stopPropagation()} className="h-8 text-[12px] font-semibold px-2.5 py-1 w-full" placeholder={placeholder} />
  );
}
  ...
  return (
    <div className="relative" onClick={(e) => e.stopPropagation()}>
      ...
      <Input value={query} onChange={(e) => onQueryChange(e.target.value)}
        ...
        placeholder={placeholder} className="h-8 text-[11px] pl-8 pr-3" />
      {showDropdown && (
        <div className="absolute z-20 w-full mt-1 bg-popover border rounded-lg shadow-lg max-h-48 overflow-auto">
          {query.length > 0 && results.length === 0 && <div className="px-3 py-2 text-[11px] text-muted-foreground">No matches found</div>}
          {results.map((r) => (
            <button key={r.id} type="button" className="w-full px-3 py-2 text-left text-[11px] hover:bg-muted flex flex-col" onMouseDown={() => onSelect(r.id)}>
              <div className="font-medium">{r.label}</div>
              {r.sublabel && <div className="text-[10px] text-muted-foreground mt-0.5">{r.sublabel}</div>}
            </button>
          ))}
          {bottomAction && (<>
            <div className="border-t" />
            <button type="button" className="w-full px-3 py-2.5 text-left text-[11px] hover:bg-sky-50 flex items-center gap-1.5 text-sky-700 font-medium"
              onMouseDown={bottomAction.onSelect}><bottomAction.icon className="h-3.5 w-3.5" />{bottomAction.label}</button>
          </>)}
        </div>
      )}
    </div>
...
function TriageItem({ label, extractedValue, checked, onChange, status, fieldKey, onHover, editable, onValueEdit, editPlaceholder, disabled, children }: {
  ...
  return (
    <div>
      <div className={cn("flex items-start gap-3 py-2.5 px-3 rounded-lg cursor-pointer transition-all duration-100",
          isMissing && !checked && "bg-amber-50/50 border border-amber-200/60 hover:bg-amber-100/40",
          !isMissing && !checked && "border border-transparent hover:bg-accent/30 hover:border-border/30",
          checked && "bg-muted/10 border border-transparent hover:bg-muted/20",
          flash && "!bg-violet-50/80 !border-violet-300/60 transition-none",
        )}
        onClick={() => handleCheck(!checked)}
        onMouseEnter={() => region && onHover?.(region)}
        onMouseLeave={() => onHover?.(null)}>
        <div className="shrink-0 mt-0.5"><Checkbox checked={checked} onCheckedChange={(v) => handleCheck(!!v)} className="h-[18px] w-[18px]" onClick={(e) => e.stopPropagation()} /></div>
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center justify-between gap-2">
            <span className={cn("text-[12px] font-medium leading-snug", checked ? "text-muted-foreground" : "text-foreground/80")}>{label}</span>
...
function SectionHeader({ icon: Icon, iconColor, bgColor, title }: { icon: React.ElementType; iconColor: string; bgColor: string; title: string }) {
  return (
...
export function TriageChecklist({ fax, onApprove, onRequestInfo, onReject, onHighlightChange }: TriageChecklistProps) {
  ...
  const selectPatient = (patId: string) => {
    const p = PATIENT_DIRECTORY.find(x => x.id === patId); if (!p) return;
  ...
  const selectPhysician = (physId: string) => {
    const p = PHYSICIAN_DIRECTORY.find(x => x.id === physId); if (!p) return;
    ...
    <div className="text-sm">
      ...
      <div className="px-4 py-3.5 border-b border-border/40">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={cn("font-mono text-[13px] font-bold rounded-md px-1.5 py-0.5", progress === 100 ? "text-emerald-600 bg-emerald-50" : "text-foreground")}>{completedChecks}/{totalChecks}</span>
        </div>
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div className={cn("h-full rounded-full transition-all duration-500 ease-out", progress === 100 ? "bg-emerald-500" : "bg-sky-500")} style={{ width: `${progress}%` }} />
        </div>
      </div>

      {/* Urgency */}
      <section className="px-4 py-3.5 border-b border-border/40">
        <SectionHeader icon={Zap} iconColor="text-red-500" bgColor="bg-red-50" title="Urgency" />
        <div className={cn("py-2.5 px-3 rounded-lg transition-all border space-y-2",
          !urgency ? "bg-amber-50/50 border-amber-200/60" : "border-transparent hover:bg-accent/30"
        )} onMouseEnter={() => onHighlightChange?.(FIELD_REGIONS.urgency)} onMouseLeave={() => onHighlightChange?.(null)}>
          <div className="flex items-center gap-3">
            <Checkbox checked={checks.urgency} onCheckedChange={() => toggle("urgency")} className="h-[18px] w-[18px]" />
            <div className="flex-1 flex items-center justify-between">
              <span className="text-[12px] font-medium text-foreground/80">Urgency status</span>
          ...
          <div className="ml-[30px]">
            <Select value={urgency} onValueChange={(v) => {
              ...
              <SelectTrigger className="h-8 w-44 text-[12px] font-semibold" onClick={(e) => e.stopPropagation()}>
            ...
            {urgency === "urgent" && (
              <div className="mt-2 flex items-center gap-2.5" onMouseEnter={() => onHighlightChange?.(FIELD_REGIONS.urgencyReason)} onMouseLeave={() => onHighlightChange?.(null)}>
                <span className="text-[11px] text-muted-foreground font-semibold shrink-0">Reason</span>
      ...
      {/* Patient Information */}
      <section className="px-4 py-3.5 border-b border-border/40">
        <SectionHeader icon={User} iconColor="text-violet-600" bgColor="bg-violet-50" title="Patient Information" />
        <div className="space-y-1.5">
          <div className={cn("py-2.5 px-3 rounded-lg border transition-all", !checks.patientMatch ? "bg-amber-50/50 border-amber-200/60" : "border-transparent hover:bg-accent/30")}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[12px] font-medium text-foreground/80">{matchedPatient ? `Patient: ${matchedPatient.name}` : isNewPatient ? "New patient (not in EMR)" : "Patient match"}</span>
            ...
            {isNewPatient && (
              <div className="mt-2 px-3 py-2.5 rounded-lg bg-sky-50 border border-sky-200/80">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold text-sky-800"><UserPlus className="h-3.5 w-3.5" />New patient chart created on approval</div>
                <p className="text-[10px] text-sky-700/70 mt-0.5 leading-relaxed">Verify the extracted info below, then approve.</p>
              </div>
            )}
          </div>
            ...
            {hasDuplicates && (
              <div className="ml-[30px] mt-1.5">
                <button type="button" onClick={() => setDuplicateExpanded(!duplicateExpanded)} className="flex items-center gap-1 text-[11px] text-red-600 font-medium hover:underline">
                  <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", duplicateExpanded && "rotate-180")} />{fax.duplicateReferralIds!.length} duplicate found — review
                </button>
                {duplicateExpanded && <div className="mt-2">{duplicateFaxes.map(dup => (
                  <div key={dup.id} className="rounded-lg border border-red-200/80 bg-red-50/50 p-3">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">{dup.description}</p>
                    <button type="button" className="flex items-center gap-1 text-[10px] text-sky-600 font-medium mt-1.5 hover:underline"><ExternalLink className="h-3 w-3" />View duplicate</button>
                  </div>
                ))}</div>}
              </div>
            )}
      ...
      <section className="px-4 py-3.5 border-b border-border/40">
        <SectionHeader icon={Stethoscope} iconColor="text-sky-600" bgColor="bg-sky-50" title="Physician Information" />
        <div className="space-y-1.5">
          <div className={cn("py-2.5 px-3 rounded-lg border transition-all", !checks.refPhysician && !matchedPhysician ? "bg-amber-50/50 border-amber-200/60" : "border-transparent hover:bg-accent/30")}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[12px] font-medium text-foreground/80">{matchedPhysician ? `Physician: ${matchedPhysician.name}` : isNewPhysician ? "New physician (not in Salesforce)" : "Referring physician"}</span>
            ...
            {isNewPhysician && !matchedPhysician && (
              <div className="mt-2 px-3 py-2.5 rounded-lg bg-amber-50/80 border border-amber-200/80">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-800"><UserPlus className="h-3.5 w-3.5" />Physician not in Salesforce</div>
                <p className="text-[10px] text-amber-700/70 mt-0.5">New Salesforce entry created on approval.</p>
              </div>
            )}
          </div>
      ...
      <section className="px-4 py-3.5 border-b border-border/40">
        <SectionHeader icon={FileText} iconColor="text-amber-600" bgColor="bg-amber-50" title="Clinical Information" />
        <div className="space-y-1.5">
      ...
      <section className="px-4 py-3.5 border-b border-border/40">
        <SectionHeader icon={allEligible ? ShieldCheck : ShieldAlert} iconColor={allEligible ? "text-emerald-600" : "text-amber-600"} bgColor={allEligible ? "bg-emerald-50" : "bg-amber-50"} title="Initial KMH Eligibility Screen" />
        {!Object.values(eligibility).every(Boolean) && !eligibilityOverride && (
          <div className="mb-3 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200/80 text-[12px] text-red-700 font-medium">
        ...
        <div className="space-y-0.5">
          {[{ label: "Patient age ≥ 12 years", ok: eligibility.ageEligible }, { label: "Patient weight < 300 lbs", ok: eligibility.weightEligible }].map(item => (
            <div key={item.label} className="flex items-center gap-3 py-2 px-3 text-[12px] font-medium rounded-lg">
              {item.ok ? <Check className="h-4 w-4 text-emerald-500 shrink-0" /> : <X className="h-4 w-4 text-red-500 shrink-0" />}
              <span className={item.ok ? "text-muted-foreground" : "text-red-700"}>{item.label}</span>
            </div>
          ))}
        </div>
        {!Object.values(eligibility).every(Boolean) && (
          <div className="mt-2.5">
            {!showOverrideInput ? (
              <Button variant="outline" size="sm" className="h-8 text-[11px] border-amber-200 text-amber-700 hover:bg-amber-50" onClick={() => setShowOverrideInput(true)}>
                <ShieldAlert className="h-3.5 w-3.5 mr-1.5" />Manual Override
              </Button>
            ) : (
              <div className="space-y-2 p-3 rounded-lg border border-amber-200/80 bg-amber-50/50">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="h-8 text-[11px] bg-amber-600 hover:bg-amber-700 text-white" disabled={!overrideReason.trim()}
                    onClick={() => { setEligibilityOverride(true); setShowOverrideInput(false); }}>Confirm Override</Button>
                  <Button variant="ghost" size="sm" className="h-8 text-[11px]" onClick={() => { setShowOverrideInput(false); setOverrideReason(""); }}>Cancel</Button>
                </div>
              </div>
            )}
            {eligibilityOverride && (
              <div className="mt-2 px-3 py-2.5 rounded-lg bg-amber-50/80 border border-amber-200/80">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-800"><ShieldAlert className="h-3.5 w-3.5" />Eligibility overridden</div>
                <p className="text-[10px] text-amber-700/70 mt-0.5">Reason: {overrideReason}</p>
              </div>
            )}
          </div>
        )}
      ...
      <section className="px-4 py-4">
        <div className="flex flex-col gap-2.5">
          <Button className="w-full h-10 bg-emerald-600 hover:bg-emerald-700 text-white text-[12px] font-semibold rounded-lg" disabled={!canApprove} onClick={onApprove}>
            <Check className="h-4 w-4 mr-2" />
            {isNewPatient ? "Approve — Create Patient & Send to Screening" : "Approve — Send to Screening"}
          </Button>
          <Button variant="outline" className="w-full h-10 border-amber-200 text-amber-700 hover:bg-amber-50 text-[12px] font-semibold rounded-lg" onClick={onRequestInfo}>
            <AlertTriangle className="h-4 w-4 mr-2" />Request Missing Info
          </Button>
          <Button variant="outline" className="w-full h-10 border-red-400 text-red-700 hover:bg-red-50 text-[12px] font-semibold rounded-lg" onClick={onReject}>
            <X className="h-4 w-4 mr-2" />Reject Referral
          </Button>
        </div>
        {!canApprove && allEligible && <p className="text-[11px] text-muted-foreground mt-3 text-center">Complete all {totalChecks} items to approve</p>}
      </section>
    </div>
  );
}
